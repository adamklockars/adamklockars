...
def start(request):
    ...
    g.player1 = request.POST['player1'] if not request.POST['player1'] == '' else request.POST['piece1']
    g.piece1 = request.POST['piece1']
    g.player2 = request.POST['player2']
    if request.POST['piece1'] == 'X':
        g.player2 = request.POST['player2'] if not request.POST['player2'] == '' else 'O'
        g.piece2 = 'O'
        g.last_move = 'O'
    else:
        g.player2 = request.POST['player2'] if not request.POST['player2'] == '' else 'X'
        g.piece2 = 'X'
        g.last_move = 'X'
    g.save()
